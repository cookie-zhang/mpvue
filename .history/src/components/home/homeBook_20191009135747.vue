      ...
      <div class="home-book-row" v-for="(item, index) in bookData" :key="index">
        <div class="home-book-col" :style="{flex: '0 0'+(100/col)+'%'}" v-for="(book,bookIndex) in item" :key="bookIndex">
           <div class="book-wraper" >
             ...
             <div class="book-title-wraper  book-title-col" v-if="HOME_BOOK_MODE === 'row'">
                <div class="book-title">{{book.title}}</div>
             </div>
             <div class="book-title-wraper  book-title-row" >
                <div class="book-title">{{book.title}}</div>
                <div class="book-author">{{book.author}}</div>
                <div class="book-categoryText">{{book.categoryText}}</div>
             </div>
             <div></div>
           </div>
        </div>
